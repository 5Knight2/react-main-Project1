...
const deleteItem=(id)=>{
  setItems((prevState)=>{
    initial=prevState.filter((item)=>item.id!=id)
    return(initial)
  })
}
  ...
  const [items,setItems]=useState(initial)
   return (
      <React.Fragment>
        <main>
          <Main items={items} addItem={addItem} deleteItem={deleteItem}></Main>
        </main>
      </React.Fragment>
  );
}
